...
<head>
    ...
    <title>Edit Transaksi - Rental System</title>
...
<body class="bg-light">
            ...
            <div class="col-md-8">
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-white py-3">
                        <h5 class="mb-0 fw-bold">Edit Transaksi Penyewaan</h5>
                    </div>
                    <div class="card-body p-4">
                        ...
                        <form action="{{ route('penyewaan.update', $penyewaan->id) }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="kode_transaksi" class="form-control" value="{{ $penyewaan->kode_transaksi }}" readonly>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="pelanggan_id" class="form-select" required>
                                    ...
                                    @foreach($pelanggan as $item)
                                        <option value="{{ $item->id }}" {{ old('pelanggan_id', $penyewaan->pelanggan_id) == $item->id ? 'selected' : '' }}>
                                ...
                                <select name="barang_id" id="barang_id" class="form-select" required>
                                    ...
                                    @foreach($barang as $item)
                                        <option value="{{ $item->id }}" 
                                            ...
                                            data-stok="{{ $item->stok }}"
                                            {{ old('barang_id', $penyewaan->barang_id) == $item->id ? 'selected' : '' }}>
                                            {{ $item->nama_barang }} — Rp {{ number_format($item->harga_sewa, 0, ',', '.') }}/hari (Stok: {{ $item->stok }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="jumlah" id="jumlah" class="form-control" min="1" value="{{ old('jumlah', $penyewaan->jumlah) }}" required>
                            </div>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="date" name="tgl_sewa" id="tgl_sewa" class="form-control" value="{{ old('tgl_sewa', $penyewaan->tgl_sewa) }}" required>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label">Tanggal Kembali</label>
                                    <input type="date" name="tgl_kembali" id="tgl_kembali" class="form-control" value="{{ old('tgl_kembali', $penyewaan->tgl_kembali) }}" required>
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="status" class="form-select" required>
                                    <option value="Diproses" {{ old('status', $penyewaan->status) == 'Diproses' ? 'selected' : '' }}>Diproses</option>
                                    <option value="Disewa" {{ old('status', $penyewaan->status) == 'Disewa' ? 'selected' : '' }}>Disewa</option>
                                    <option value="Dikembalikan" {{ old('status', $penyewaan->status) == 'Dikembalikan' ? 'selected' : '' }}>Dikembalikan</option>
                            ...
                            <div class="mb-4 p-3 bg-light rounded border">
                                <label class="form-label fw-bold text-muted mb-0">Total Biaya</label>
                                <h3 class="fw-bold text-success mb-0" id="display_total">Rp {{ number_format($penyewaan->total_harga, 0, ',', '.') }}</h3>
    ...
    <script>
        ...
        // Hitung ulang saat pertama kali memuat halaman
        hitungTotal();
    </script>
